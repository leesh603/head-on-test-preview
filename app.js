...
import {fx,fxReady,fxTint,fxCell} from './fx-art.js?v=201';
import {MG_TIER_COLUMN,MG_TIER_ROW} from './mg-fx-tiers.js?v=201';
...
import {drawStageBoss,updateStageBossHud,paintCity,paintSky,prepareStageBossAssets} from './stageboss-view.js?v=195';
import {enableStageBoss,STAGE_NAMES} from './stageboss-host.js?v=199';
import './hud-layout94.js?v=180';
import {showBattlefieldEvent,hideBattlefieldEvent} from './battlefield-event-ui.js?v=176';
import {CoopGame,coopPlane} from './coop-engine.js?v=201';
import {CoopInput,coopRecord,saveCoopLocal,COOP_RECORD_KEYS} from './coop-input.js?v=163';
import {drawCoop} from './coop-view.js?v=201';
import {drawSunStrike} from './sun-strike71.js?v=201';
...
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft} from './campaign.js?v=201';
import {drawCampaign} from './campaign-view.js?v=201';
...
import {sfx,setSfxMuted} from './sfx.js?v=199';
...
import{planeSprite,planePreview,aircraftReady,aircraftKey}from'./aircraft.js?v=195';
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale}from'./engine.js?v=201';
...
function draw(t){let cx=game?game.x:Math.sin(t*.03)*45,cy=game?game.y:-t*12;ctx.save();if(game?.shake)ctx.translate((Math.random()-.5)*game.shake,(Math.random()-.5)*game.shake);terrain(cx,cy);drawStageBoss(ctx,game,W,H,{drawZeppelin,drawFieldArt,layer:'bodies'});if(game){ctx.strokeStyle='#ecf6de35';ctx.lineWidth=1;for(let i=0;i<16;i++){let px=(i*137.3+t*73)%W,py=(i*97.7+t*170)%H;ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px-Math.cos(game.a)*22,py-Math.sin(game.a)*22);ctx.stroke()}}const point=(x,y)=>[x-cx+W/2,y-cy+H/2];if(game){
 ...
 for(const e of game.enemies||[])for(const ghost of e.vossTrails||[]){const [gx,gy]=point(ghost.x,ghost.y);ctx.save();ctx.globalAlpha=.34*(ghost.life/ghost.maxLife);planeSprite(ctx,gx,gy,ghost.a,e.bossPlane||'fokker_voss',enemyAircraftScale(e),true,false);ctx.restore()}
 for(let d of game.drops){let[x,y]=point(d.x,d.y);if(d.heal||d.supply){ctx.strokeStyle='#9cffb4';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,22+Math.sin(t*5)*3,0,Math.PI*2);ctx.stroke();drawEquipment(ctx,'repair',x,y+Math.sin(t*3)*2,0,40)}else{ctx.fillStyle='#63d5ec';ctx.fillRect(x-3,y-3,6,6)}}for(const grenade of game.grenades||[]){const[gx,gy]=point(grenade.x,grenade.y);drawGrenade(ctx,grenade,gx,gy)}for(let m of game.mines){let[x,y]=point(m.x,m.y);if(!fx(ctx,'mine',x,y,30,30))drawEquipment(ctx,'mine',x,y,0,30);if(m.arm===0){ctx.strokeStyle='#ffcc6677';ctx.lineWidth=1;ctx.beginPath();ctx.arc(x,y,20+Math.sin(t*5)*2,0,Math.PI*2);ctx.stroke()}}for(let b of game.bullets){if(b.flak||b.enemy||b.motorCannon||b.cow37)continue;let[x,y]=point(b.x,b.y);if(b.rocket){if(!fx(ctx,'rocket',x,y,52,16,Math.atan2(b.vy,b.vx)))drawEquipment(ctx,'rocket',x,y,Math.atan2(b.vy,b.vx)+Math.PI/2,48);}else if(b.fonckSeeker){ctx.save();ctx.translate(x,y);ctx.rotate(Math.atan2(b.vy,b.vx));if(!fx(ctx,'tracerCream',-6,0,46,10)){ctx.fillStyle='#fff5bd';ctx.fillRect(-18,-3,30,6);ctx.fillStyle='#f3a94f';ctx.fillRect(-25,-1,12,2)}ctx.restore();}else if(b.mg){const ta=Math.atan2(b.vy,b.vx),sp=Math.hypot(b.vx,b.vy)||1,ux=b.vx/sp,uy=b.vy/sp;if(!fxCell(ctx,'mgTiers',MG_TIER_COLUMN[game.mgFxTier?game.mgFxTier():'NORMAL']??0,MG_TIER_ROW.tracer,64,64,x-ux*13,y-uy*13,16,44,ta-Math.PI/2)){ctx.strokeStyle='#f8df87';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x-b.vx*.018,y-b.vy*.018);ctx.stroke()}}else{const bc=b.mauserRound?'#a98cff':b.enemy?'#ffb096':(b.formation||b.ally)?'#b9f2de':b.pierce?'#f8f5cd':'#f8df87',bw=b.enemy?3:(b.formation||b.ally)?4:b.pierce?3:2;if(!fxTint(ctx,'tracerAmber',bc,x,y,bw*11,bw*4,Math.atan2(b.vy,b.vx))){ctx.strokeStyle=bc;ctx.lineWidth=bw;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x-b.vx*.018,y-b.vy*.018);ctx.stroke()}}}for(let e of game.enemies){let[x,y]=point(e.x,e.y);if(e.groundEscort){drawBattlefieldSprite(ctx,'aa',x,y,96,e.a+Math.PI/2);continue}if(e.stageBossBody||e.heavyBomber||e.missionGround||e.navalVessel||e.fieldUnit)continue;let key=e.escortPlane?e.escortPlane:e.bossPlane?e.bossPlane:e.ace&&faction==='entente'?'fokker_red':faction==='central'?(e.type==='hunter'?'nieuport':e.type==='boss'?'spad':'camel'):(e.type==='hunter'||e.type==='boss'?'fokker':'albatros');let size=enemyAircraftScale(e);planeSprite(ctx,x+14,y+22,e.a,key,size,true,true);planeSprite(ctx,x,y,e.a,key,size,true,false,e.hitFlash,e.hp<=e.maxHp*.5);if(e.vossInvuln>0||e.aceInvuln124>0){ctx.save();ctx.globalAlpha=.55;ctx.strokeStyle=e.aceInvuln124>0?'#35253d':'#e9f2ca';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,42+Math.sin(t*20)*4,0,Math.PI*2);ctx.stroke();ctx.restore()}if(e.type==='zeppelin')drawZeppelin(ctx,x,y,e.a,size,e.hitFlash>0);if(e.type==='boss'){ctx.fillStyle='#2a3024';ctx.fillRect(x-30,y-40,60,4);ctx.fillStyle='#e7835c';ctx.fillRect(x-30,y-40,60*e.hp/e.maxHp,4)}}for(let p of game.particles){let[x,y]=point(p.x,p.y);const smokeLife=p.smoke?Math.max(0,p.life/p.maxLife):0;ctx.globalAlpha=p.smoke?smokeLife*(p.muzzleSmoke?.88:.65):Math.min(1,p.life*3);if(p.hitFx){ctx.globalAlpha=Math.min(1,p.life*8);fxCell(ctx,'mgTiers',MG_TIER_COLUMN[p.mgTier||'NORMAL']??0,p.hitFx==='armor'?MG_TIER_ROW.hit_armor:MG_TIER_ROW.hit_air,64,64,x,y,32,32);ctx.globalAlpha=1;continue}if(p.petal){drawPetalParticle(ctx,p,x,y);continue}if(p.heart){drawPixelHeart(ctx,Math.round(x),Math.round(y),p.size,p.color);continue}ctx.fillStyle=p.color;let size=p.smoke?p.size+(1-smokeLife)*(p.muzzleSmoke?15:10):3;
...
drawSupport=()=>{
 ...
 for(const f of game.hostileMinefields||[]){const[x,y]=point(f.x,f.y);ctx.save();ctx.strokeStyle=f.warning>0?'#ffe0a199':'#e58b6c88';ctx.lineWidth=2;ctx.setLineDash([6,9]);ctx.beginPath();ctx.arc(x,y,f.radius,0,6.283);ctx.stroke();ctx.setLineDash([]);ctx.font='bold 11px sans-serif';ctx.textAlign='center';ctx.fillStyle='#ffd2a9';ctx.fillText(f.warning>0?(getLocale()==='en'?'Mine active '+f.warning.toFixed(1)+'s':'기뢰 활성화 '+f.warning.toFixed(1)+'초'):(getLocale()==='en'?'Minefield':'기뢰지대'),x,y-f.radius-9);for(const m of f.mines){if(m.dead)continue;const[mx,my]=point(m.x,m.y);if(!fx(ctx,'mine',mx,my,30,30))drawEquipment(ctx,'mine',mx,my,0,28);ctx.strokeStyle=f.warning>0?'#ffe0a188':'#ff876e';ctx.beginPath();ctx.arc(mx,my,18,0,6.283);ctx.stroke()}ctx.restore()}
...
draw=t=>{
 ...
 for(const b of game.bullets){
  ...
  if(b.rocket){const a=Math.atan2(b.vy,b.vx);ctx.strokeStyle='#ff982baa';ctx.lineWidth=b.special?5:3;ctx.beginPath();ctx.moveTo(x-Math.cos(a)*15,y-Math.sin(a)*15);ctx.lineTo(x-Math.cos(a)*34,y-Math.sin(a)*34);ctx.stroke();continue}
...
// xp-gem.png intentionally unused: drops draw the procedural cyan square.
...
const _hud51=hud;
hud=()=>{_hud51();const held=UPGRADES.filter(u=>u.legendary&&game.upgrades[u.id]);const signature=held.map(u=>u.id).join(',');if(signature!==legendarySignature){legendarySignature=signature;$('legendaryInventory').replaceChildren();for(const u of held){const item=document.createElement('button');item.type='button';item.className='legendary-badge';item.setAttribute('aria-label',reinforcementName(u,game,PLANES)+' · '+cleanDescription(u.desc));const tip=document.createElement('span');tip.className='legendary-tip156';tip.textContent=reinforcementName(u,game,PLANES)+' · '+cleanDescription(u.desc);const c=document.createElement('canvas');c.width=48;c.height=48;const key=u.id==='ironCross'?'ironCross-'+(PLANES[game.plane]?.faction==='entente'?'entente':'central'):u.id;drawGameIcon(c.getContext('2d'),key,24,24,42);item.append(c,tip);item.onclick=e=>{e.preventDefault();item.classList.toggle('tip156')};item.onblur=()=>item.classList.remove('tip156');$('legendaryInventory').append(item)}}};
const _draw51=draw;
...
function renderCoopSetup(){
 ...
 $('coopSkill2').textContent=pilotLoadout(coopPilot2,coopSelectedPlane(coopPilot2)).skill;const c=$('coopPlane2').getContext('2d');c.clearRect(0,0,144,160);planePreview(c,72,80,aircraftKey(coopSelectedPlane(coopPilot2),false,coopPilot2),126,140);
...
function coopHud(){
 ...
 for(const p of g.players){const id=p.id,down=p.status==='downed';$(id+'HudName').textContent=id.toUpperCase()+' · '+p.nickname+' / '+pilotName(p.pilot,PILOTS[p.pilot].name);const ammo=p.unlimitedAmmo?'∞':p.ammo.join(' / '),cooldown=p.cooldown>0?p.cooldown.toFixed(1)+'s':t('coop.ready'),special=p.specialAmmoStatus?.();$(id+'HudStats').textContent=down?t('coop.downed',{seconds:p.respawnRemaining.toFixed(1),level:p.level}):t('coop.stats',{hp:Math.ceil(p.hp),maxHp:Math.round(p.maxHp),level:p.level,ammo,reload:p.reloadTime>0?t('coop.reload',{seconds:p.reloadTime.toFixed(1)}):'',special:special?t('coop.special',{name:special.name,count:special.count}):'',active:cooldown,maneuver:p.evadeCooldown>0?p.evadeCooldown.toFixed(1)+'s':t('coop.ready')});$(id+'HpBar').style.width=100*p.hp/p.maxHp+'%';
  const held=UPGRADES.filter(u=>u.legendary&&p.upgrades[u.id]),signature=g.runId+':'+held.map(u=>u.id).join(',');if(coopRelicSignatures[id]!==signature){coopRelicSignatures[id]=signature;$(id+'Relics').replaceChildren();for(const u of held){const icon=document.createElement('canvas');icon.width=48;icon.height=48;icon.title=reinforcementName(u,p,PLANES)+' · '+cleanDescription(u.desc);icon.setAttribute('aria-label',reinforcementName(u,p,PLANES));const key=u.id==='ironCross'?'ironCross-'+(PLANES[p.plane]?.faction==='entente'?'entente':'central'):u.id;drawGameIcon(icon.getContext('2d'),key,24,24,42);$(id+'Relics').append(icon)}}
...
roster=()=>{
 ...
 $('pilotName').textContent=pilotLabel;$('skillName').textContent=localizedActive(loadoutId,loadout.skill);$('skillDesc').textContent=localizedPilotDescription(loadoutId,loadout.desc);
 $('passive103').textContent=localizedPassive(loadoutId,loadout.passive)+' · '+localizedPassiveDescription(loadoutId,loadout.passiveDesc);
