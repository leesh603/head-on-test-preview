...
function drawPilotPassives(g,x,y,t,point){
 ...
 if((pil==='baracca'||pil==='immelmann')&&pl.weapon?.guns===1&&pl.muzzleFlash>0){ctx.save();const nx=x+Math.cos(g.a)*30,ny=y+Math.sin(g.a)*30;ctx.globalAlpha=.9;ctx.fillStyle='#ff9a3c';ctx.beginPath();ctx.arc(nx,ny,9+Math.sin(t*40)*2,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff1c0';ctx.beginPath();ctx.arc(nx,ny,4,0,Math.PI*2);ctx.fill();ctx.restore()}
 // Jacobs: little black devils, one per kill-stack (max 3)
 if(pil==='immelmann'&&(pl.immerBoost||0)>0){ctx.save();const f=Math.min(1,pl.immerBoost);ctx.globalAlpha=.5+.3*Math.sin(t*14);ctx.strokeStyle='#7fd4ff';ctx.lineWidth=2.4;ctx.beginPath();ctx.arc(x,y,40,g.a-Math.PI/2-.9,g.a-Math.PI/2+.9);ctx.stroke();ctx.globalAlpha=.85*f;ctx.strokeStyle='#cfeeff';ctx.lineWidth=1.3;ctx.beginPath();ctx.arc(x,y,34,g.a-Math.PI/2-.65,g.a-Math.PI/2+.65);ctx.stroke();ctx.restore()}
