import {drawGrenade,drawGrenadeBlast,drawAmatolBlast,drawFxExplosion} from './weapon-effects156.js?v=307&b=307';
import {fx,fxReady,fxTint,FX56} from './fx-art.js?v=307';
import {CATEGORIES,categoryName,reinforcementName,buildStats,cumulativeText,cleanDescription} from './reinforcement-ui151.js?v=307';
import {t,getLocale,setLocale,subscribe,initLocale,applyTranslations,rarityName,upgradeDescription as translatedUpgradeDescription,pilotName,aircraftName,weaponName,activeName,passiveName,pilotDescription,passiveDescription,aircraftRole,airframeHistory,airframeTip} from './i18n.js?v=307';
import {GamepadInput} from './gamepad-input.js?v=307';
...
import {playerPose,drawPlayerAura,drawPetalParticle,drawRedGhosts162} from './player-effects129.js?v=307&b=307';
import {drawStageBoss,updateStageBossHud,paintCity,paintSky,prepareStageBossAssets} from './stageboss-view.js?v=307&b=307';
import {enableStageBoss,stageBossBounds,harborBankOffset} from './stageboss-host.js?v=307&b=307';
import {chooseTransitionTip,transitionRegionLabel} from './transition-tips188.js?v=307';
import './hud-layout94.js?v=307';
import {showBattlefieldEvent,hideBattlefieldEvent} from './battlefield-event-ui.js?v=307';
import {CoopGame,coopPlane} from './coop-engine.js?v=307&b=307';
import {CoopInput,coopRecord,saveCoopLocal,COOP_RECORD_KEYS} from './coop-input.js?v=307';
import {drawCoop} from './coop-view.js?v=307&b=307';
import {drawSunStrike} from './sun-strike71.js?v=307&b=307';
import {drawEnemyProjectile,drawCannonProjectile,drawBattlefieldFire,friendlyTracerColor} from './projectiles.js?v=307&b=307';
import {installFlightViewport} from './flight-viewport.js?v=307';
import {aircraftFeelRatings,representativeArchetypeKey} from './aircraft-feel174.js?v=307';
import {drawGas} from './gas-view.js?v=307&b=307';
import {missionNavigation,drawMissionRadar} from './navigation.js?v=307&b=307';
import {drawBattlefieldSprite} from './battlefield-art.js?v=307&b=307';
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft,liveryVariant} from './campaign.js?v=307&b=307';
import {drawCampaign} from './campaign-view.js?v=307&b=307';
import {campaignArtReady} from './aircraft.js?v=307&b=307';
import {drawGameIcon,drawSpecialAmmoIcon,iconsReady} from './icons.js?v=307';
import {BattleMusic,musicModeForGame} from './music.js?v=307&b=307';
import {sfx,setSfxMuted} from './sfx.js?v=307&b=307';
import {portraitSources,portraitsReady} from './portraits.js?v=307&b=307';
import {BOSS_CATALOG} from './headon-stageboss-patterns.js?v=307&b=307';
import {drawEquipment} from './equipment.js?v=307&b=307';
import {installHeadOnElitePatch,createEliteAssets,renderEliteLayer} from './elite-patch/module/index.js?v=307';
import{planeSprite,aircraftReady,aircraftKey,hangarArtReady}from './aircraft.js?v=307&b=307';
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale,LEGENDARY_DEFENSE_BALANCE,LEGENDARY_BALANCE}from './engine.js?v=307&b=307';
import {AUGMENTATION_OVERHAUL_BALANCE}from'./augmentation-overhaul150.js?v=307';
import {drawCloudCover}from'./cloud-cover1.js?v=307&b=307';
import {drawFleetLayer}from'./fleet-naval1.js?v=307&b=307';
import {drawTrenchLayer}from'./trench-war1.js?v=307&b=307';
import {drawCityAirLayer}from'./city-air1.js?v=307&b=307';
import {TerrainRenderer,MountainField} from './alps-terrain117.js?v=307';
const flightViewport=installFlightViewport(document,window);
const ententeAirshipSprite=new Image();ententeAirshipSprite.src='./zeppelin-entente.webp?v=307&b=307';
const zeppelinSprite=new Image();zeppelinSprite.src='./zeppelin.webp?v=307&b=307';const ballCloudSprite=new Image();ballCloudSprite.src='./fx-ball-cloud.webp?v=307&b=307';
...
const eliteAssets=createEliteAssets({
 lePrieur:'./elite-patch/assets/le-prieur-squadron.webp?v=307',
 schlachtstaffel:'./elite-patch/assets/halberstadt-cliv-squadron.webp?v=307',
...
function renderRankingMedals(rows,heading=t('ranking.serverPriority')){
 ...
 const list=document.createElement('ol');list.className='ranking-medal-list';rows.forEach((r,i)=>{const row=document.createElement('li'),badge=document.createElement(i===0?'img':'canvas');badge.className='ranking-medal';
  if(i===0){badge.src='./medal55.webp?v=307&b=307';badge.alt='1 · '+t('ranking.medal.centralDisplay');}else{badge.width=42;badge.height=42;badge.setAttribute('aria-label',(i+1)+' · '+(i===1?t('ranking.medal.ironCross'):t('ranking.medal.ententeDisplay')));drawGameIcon(badge.getContext('2d'),i===1?'ironCross':'victoriaCross',21,21,38);}
...
function fieldArtImg(key){if(!key)return null;let im=fieldArt[key];if(!im){im=new Image();im.src=`./${FIELD_ART_FILE[key]||key}.webp?v=307&b=307`;fieldArt[key]=im}return im.naturalWidth?im:null}
...
const horseEmblem=new Image();horseEmblem.src='./horse-emblem.webp?v=307&b=307';
const xpGem=null;
...
const regionTextures={};for(const name of ['sea','trenches']){const im=new Image();im.src='./terrain-'+name+'.webp?v=307';regionTextures[name]=im}
const terrainAlpsAtlas=new Image();terrainAlpsAtlas.src='./terrain-alps-atlas.webp?v=307';
...
const zeeWaterTile=new Image();zeeWaterTile.src='./terrain-zeebrugge-water.webp?v=307';
const zeebruggeHarborArt=new Image();zeebruggeHarborArt.src='./terrain-zeebrugge-route.webp?v=307';
...
const BOSS_CUTIN_ART={'paris-gun':'boss-art-paris-gun.webp?v=307&b=307','lincomparable':'boss-art-lincomparable.webp?v=307&b=307','sms-stuttgart':'boss-art-sms-stuttgart.webp?v=307&b=307','hms-zubian':'boss-art-hms-zubian.webp?v=307&b=307','a7v-flak':'boss-art-a7v-flak.webp?v=307&b=307','mark-v-cruiser':'boss-art-mark-v-cruiser.webp?v=307&b=307','livens-flame-projector':'boss-art-livens-flame-projector.webp?v=307&b=307','minenwerfer-battery':'boss-art-minenwerfer-battery.webp?v=307&b=307','drachen-net':'boss-art-drachen-net.webp?v=307&b=307','london-apron':'boss-art-london-apron.webp?v=307&b=307','zeppelin-l70':'boss-art-zeppelin-l70.webp?v=307&b=307','hma23':'boss-art-hma23.webp?v=307&b=307','gik':'boss-art-gik.webp?v=307&b=307','ca4':'boss-art-ca4.webp?v=307&b=307','armored-harbor-fortress':'boss-art-armored-harbor-fortress.webp?v=307&b=307'};;
...
function renderCoopSetup(){
 ...
 $('coopPortrait2').src=portraitSources[coopPilot2]||'portrait-'+coopPilot2+'.webp?v=307&b=307';$('coopPortrait2').alt=PILOTS[coopPilot2].name;$('coopPortrait2').style.setProperty('--pilot-choice-scale',choicePortraitScale(coopPilot2));
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='skill'&&e.ownerId){const p=g.player(e.ownerId),id=p.id;coopCutinEnds[id]=g.t+2.1;$(id+'CutinPortrait').src=portraitSources[p.pilot]||'portrait-'+p.pilot+'.webp?v=307&b=307';$(id+'CutinPortrait').alt=PILOTS[p.pilot].name;$(id+'CutinName').textContent=id.toUpperCase()+' · '+p.nickname;$(id+'CutinSkill').textContent=pilotLoadout(p.pilot,p.plane).skill;sfx('skill')}
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;bossCardFrom=performance.now();$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.webp?v=307&b=307';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');bossCutinUntil=bossCardFrom+2600;setBossCutin($('bossPortrait').src,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss')}
...
const rankingMedalImages=Object.fromEntries(['central','entente'].flatMap(side=>[1,2,3].map(rank=>{
 const image=new Image();image.src=`./ranking-${side}-${rank}.webp?v=307`;return [`${side}-${rank}`,image];
})));
