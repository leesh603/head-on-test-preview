import {drawGrenade,drawGrenadeBlast,drawAmatolBlast,drawFxExplosion} from './weapon-effects156.js?v=318&b=318';
...
import {CoopGame,coopPlane} from './coop-engine.js?v=318&b=318';
import {CoopInput,coopRecord,saveCoopLocal,COOP_RECORD_KEYS} from './coop-input.js?v=318';
import {drawCoop} from './coop-view.js?v=318&b=318';
import {drawSunStrike} from './sun-strike71.js?v=318&b=318';
import {drawEnemyProjectile,drawCannonProjectile,drawBattlefieldFire,friendlyTracerColor} from './projectiles.js?v=318&b=318';
...
import {drawGas} from './gas-view.js?v=318&b=318';
import {missionNavigation,drawMissionRadar} from './navigation.js?v=318&b=318';
import {drawBattlefieldSprite} from './battlefield-art.js?v=318&b=318';
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft,liveryVariant} from './campaign.js?v=318&b=318';
import {drawCampaign} from './campaign-view.js?v=318&b=318';
import {campaignArtReady} from './aircraft.js?v=318&b=318';
import {drawGameIcon,drawSpecialAmmoIcon,iconsReady} from './icons.js?v=318';
import {BattleMusic,musicModeForGame} from './music.js?v=318&b=318';
import {sfx,setSfxMuted} from './sfx.js?v=318&b=318';
import {portraitSources,portraitsReady} from './portraits.js?v=318&b=318';
import {BOSS_CATALOG} from './headon-stageboss-patterns.js?v=318&b=318';
import {drawEquipment} from './equipment.js?v=318&b=318';
import {installHeadOnElitePatch,createEliteAssets,renderEliteLayer} from './elite-patch/module/index.js?v=318';
import{planeSprite,aircraftReady,aircraftKey,hangarArtReady}from './aircraft.js?v=318&b=318';
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale,LEGENDARY_DEFENSE_BALANCE,LEGENDARY_BALANCE}from './engine.js?v=318&b=318';
import {AUGMENTATION_OVERHAUL_BALANCE}from'./augmentation-overhaul150.js?v=318';
import {drawCloudCover}from'./cloud-cover1.js?v=318&b=318';
import {drawFleetLayer}from'./fleet-naval1.js?v=318&b=318';
import {drawTrenchLayer}from'./trench-war1.js?v=318&b=318';
import {drawCityAirLayer}from'./city-air1.js?v=318&b=318';
...
const ententeAirshipSprite=new Image();ententeAirshipSprite.src='./zeppelin-entente.webp?v=318&b=318';
const zeppelinSprite=new Image();zeppelinSprite.src='./zeppelin.webp?v=318&b=318';const ballCloudSprite=new Image();ballCloudSprite.src='./fx-ball-cloud.webp?v=318&b=318';
...
function renderRankingMedals(rows,heading=t('ranking.serverPriority')){
 ...
 const list=document.createElement('ol');list.className='ranking-medal-list';rows.forEach((r,i)=>{const row=document.createElement('li'),badge=document.createElement(i===0?'img':'canvas');badge.className='ranking-medal';
  if(i===0){badge.src='./medal55.webp?v=318&b=318';badge.alt='1 · '+t('ranking.medal.centralDisplay');}else{badge.width=42;badge.height=42;badge.setAttribute('aria-label',(i+1)+' · '+(i===1?t('ranking.medal.ironCross'):t('ranking.medal.ententeDisplay')));drawGameIcon(badge.getContext('2d'),i===1?'ironCross':'victoriaCross',21,21,38);}
...
function fieldArtImg(key){if(!key)return null;let im=fieldArt[key];if(!im){im=new Image();im.src=`./${FIELD_ART_FILE[key]||key}.webp?v=318&b=318`;fieldArt[key]=im}return im.naturalWidth?im:null}
...
const horseEmblem=new Image();horseEmblem.src='./horse-emblem.webp?v=318&b=318';
const xpGem=null;
...
const BOSS_CUTIN_ART={'paris-gun':'boss-art-paris-gun.webp?v=318&b=318','lincomparable':'boss-art-lincomparable.webp?v=318&b=318','sms-stuttgart':'boss-art-sms-stuttgart.webp?v=318&b=318','hms-zubian':'boss-art-hms-zubian.webp?v=318&b=318','a7v-flak':'boss-art-a7v-flak.webp?v=318&b=318','mark-v-cruiser':'boss-art-mark-v-cruiser.webp?v=318&b=318','livens-flame-projector':'boss-art-livens-flame-projector.webp?v=318&b=318','minenwerfer-battery':'boss-art-minenwerfer-battery.webp?v=318&b=318','drachen-net':'boss-art-drachen-net.webp?v=318&b=318','london-apron':'boss-art-london-apron.webp?v=318&b=318','zeppelin-l70':'boss-art-zeppelin-l70.webp?v=318&b=318','hma23':'boss-art-hma23.webp?v=318&b=318','gik':'boss-art-gik.webp?v=318&b=318','ca4':'boss-art-ca4.webp?v=318&b=318','armored-harbor-fortress':'boss-art-armored-harbor-fortress.webp?v=318&b=318'};;
...
function renderCoopSetup(){
 ...
 $('coopPortrait2').src=portraitSources[coopPilot2]||'portrait-'+coopPilot2+'.webp?v=318&b=318';$('coopPortrait2').alt=PILOTS[coopPilot2].name;$('coopPortrait2').style.setProperty('--pilot-choice-scale',choicePortraitScale(coopPilot2));
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='skill'&&e.ownerId){const p=g.player(e.ownerId),id=p.id;coopCutinEnds[id]=g.t+2.1;$(id+'CutinPortrait').src=portraitSources[p.pilot]||'portrait-'+p.pilot+'.webp?v=318&b=318';$(id+'CutinPortrait').alt=PILOTS[p.pilot].name;$(id+'CutinName').textContent=id.toUpperCase()+' · '+p.nickname;$(id+'CutinSkill').textContent=pilotLoadout(p.pilot,p.plane).skill;sfx('skill')}
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;bossCardFrom=performance.now();$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.webp?v=318&b=318';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');bossCutinUntil=bossCardFrom+2600;setBossCutin($('bossPortrait').src,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss')}
