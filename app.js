...
import {t,getLocale,setLocale,subscribe,initLocale,applyTranslations,rarityName,upgradeDescription as translatedUpgradeDescription,pilotName,aircraftName,weaponName,activeName,passiveName,pilotDescription,passiveDescription,aircraftRole,airframeHistory,airframeTip} from './i18n.js?v=216';
...
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale,LEGENDARY_DEFENSE_BALANCE,LEGENDARY_BALANCE}from'./engine.js?v=235';
...
function drawPilotPassives(g,x,y,t,point){
 ...
 if(pil==='bishop'){const near=(g.enemies||[]).some(e=>e.hp>0&&(e.x-pl.x)**2+(e.y-pl.y)**2<300*300);if(near){ctx.save();ctx.globalAlpha=.55;ctx.setLineDash([7,7]);ctx.strokeStyle='#e04a3a';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,300,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.restore()}}
 // Mannock: green command glints on his wingmen
 if(pil==='mannock'){const wings=(g.allies||[]).filter(a=>a.life>0);if(wings.length){ctx.save();ctx.globalAlpha=.75;wings.forEach((a,i)=>{const[ax,ay]=point(a.x,a.y);const ga=t*3+i*2.1;ctx.fillStyle=i%2?'#9ff0b0':'#6fd08a';ctx.beginPath();ctx.arc(ax+Math.cos(ga)*20,ay+Math.sin(ga)*20,2.6,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(140,240,170,.5)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(ax,ay,14+Math.sin(t*5+i)*3,0,Math.PI*2);ctx.stroke()});ctx.restore()}}
 // Barker: red chevron pips = grudge stacks
 if(pil==='barker'&&(pl.barkerStacks||0)>0){ctx.save();const n=pl.barkerStacks;ctx.globalAlpha=.9;for(let i=0;i<n;i++){const oy=y-34-i*9;ctx.fillStyle=i%2?'#ff7a5a':'#e0442e';ctx.beginPath();ctx.moveTo(x-8,oy);ctx.lineTo(x,oy-6);ctx.lineTo(x+8,oy);ctx.lineTo(x+8,oy+4);ctx.lineTo(x,oy-2);ctx.lineTo(x-8,oy+4);ctx.closePath();ctx.fill()}ctx.restore()}
 // Luke & Gontermann: markers over heavy targets in range
 if(pil==='boelcke'){const near=(g.allies||[]).filter(a=>a.life>0&&(a.x-pl.x)**2+(a.y-pl.y)**2<360*360);if(near.length){ctx.save();ctx.globalAlpha=.5;ctx.setLineDash([4,9]);ctx.strokeStyle='#a8e6c8';ctx.lineWidth=1.6;ctx.beginPath();ctx.arc(x,y,360,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);near.forEach((a,i)=>{const[ax,ay]=point(a.x,a.y);ctx.globalAlpha=.85;ctx.strokeStyle='#cfe8d8';ctx.lineWidth=1.6;ctx.beginPath();ctx.moveTo(ax-4.5,ay);ctx.lineTo(ax+4.5,ay);ctx.moveTo(ax,ay-6);ctx.lineTo(ax,ay+6);ctx.stroke();ctx.strokeStyle='#24302a';ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(ax-4.5,ay-1.5);ctx.lineTo(ax+4.5,ay-1.5);ctx.moveTo(ax-4.5,ay+1.5);ctx.lineTo(ax+4.5,ay+1.5);ctx.stroke()});ctx.restore()}}
 if(pil==='voss'){const n=(g.enemies||[]).filter(e=>e.hp>0&&(e.x-pl.x)**2+(e.y-pl.y)**2<400*400).length;if(n>=2){ctx.save();const m=Math.min(6,n);ctx.globalAlpha=.75;for(let i=0;i<m;i++){const ga=t*2.6+i*Math.PI*2/m+.4;ctx.fillStyle=i%2?'#ff9a7a':'#e0523a';ctx.beginPath();ctx.arc(x+Math.cos(ga)*44,y+Math.sin(ga)*44,2.8,0,Math.PI*2);ctx.fill()}ctx.restore()}}
 if(pil==='luke'||pil==='gontermann'){const heavy=(g.enemies||[]).filter(e=>e.hp>0&&(e.heavyBomber||e.zeppelin||e.boss||e.ace||e.big||e.stageBossBody||e.navalVessel||e.fieldUnit||e.type==='boss')&&(e.x-pl.x)**2+(e.y-pl.y)**2<700*700);if(heavy.length){ctx.save();heavy.slice(0,4).forEach((e,i)=>{const[ex,ey]=point(e.x,e.y);const r=(e.r||18)+13+Math.sin(t*6+i)*3;if(pil==='luke'){ctx.globalAlpha=.85;ctx.strokeStyle='#ffb43c';ctx.lineWidth=2;ctx.save();ctx.translate(ex,ey);ctx.rotate(t*1.5);ctx.beginPath();ctx.moveTo(0,-r);ctx.lineTo(r,0);ctx.lineTo(0,r);ctx.lineTo(-r,0);ctx.closePath();ctx.stroke();ctx.restore()}else{ctx.globalAlpha=.8;for(let j=0;j<3;j++){const ga=t*2+j*2.1;ctx.fillStyle=j%2?'#ff8a3c':'#ffce6a';ctx.beginPath();ctx.arc(ex+Math.cos(ga)*r*.7,ey+Math.sin(ga)*r*.7,2.4,0,Math.PI*2);ctx.fill()}}});ctx.restore()}}
}
...
const BOSS_CUTIN_ART={'paris-gun':'boss-bruno-train115.webp',lincomparable:'boss-lincomparable94.webp?v=116&b=117','sms-stuttgart':'boss-sms-stuttgart94.webp','hms-zubian':'boss-hms-zubian94.webp','a7v-flak':'boss-a7v-flak94.webp','mark-v-cruiser':'boss-mark-v94.webp','livens-flame-projector':'boss-livens-composite188.webp','minenwerfer-battery':'boss-minenwerfer-composite188.webp','drachen-net':'boss-drachen-net115.webp','london-apron':'boss-london-apron115.webp','zeppelin-l70':'boss-zeppelin-l7094.webp',hma23:'boss-hma2394.webp',gik:'boss-gik.webp?v=116&b=117',ca4:'boss-ca4.webp?v=116&b=117','armored-harbor-fortress':'boss-armored-harbor-main-base.webp'};
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;$('bossWarnSub').textContent=e.name+' · ENEMY ACE INBOUND';show('bossWarning');setTimeout(()=>show('bossWarning',false),1600);bossCardFrom=performance.now()+1400;$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.webp?v=214&b=218';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');bossCutinUntil=bossCardFrom+2600;setBossCutin($('bossPortrait').src,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss')}
