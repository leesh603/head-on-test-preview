...
import {drawStageBoss,updateStageBossHud,paintCity,paintSky,prepareStageBossAssets} from './stageboss-view.js?v=297&b=297';
import {enableStageBoss,stageBossBounds} from './stageboss-host.js?v=297&b=297';
...
import {CoopGame,coopPlane} from './coop-engine.js?v=297&b=297';
import {CoopInput,coopRecord,saveCoopLocal,COOP_RECORD_KEYS} from './coop-input.js?v=293';
import {drawCoop} from './coop-view.js?v=297&b=297';
import {drawSunStrike} from './sun-strike71.js?v=297&b=297';
...
import {drawGas} from './gas-view.js?v=297&b=297';
import {missionNavigation,drawMissionRadar} from './navigation.js?v=297&b=297';
import {drawBattlefieldSprite} from './battlefield-art.js?v=297&b=297';
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft,liveryVariant} from './campaign.js?v=297&b=297';
import {drawCampaign} from './campaign-view.js?v=297&b=297';
import {campaignArtReady} from './aircraft.js?v=297&b=297';
import {drawGameIcon,drawSpecialAmmoIcon,iconsReady} from './icons.js?v=293';
import {BattleMusic,musicModeForGame} from './music.js?v=297&b=297';
import {sfx,setSfxMuted} from './sfx.js?v=297&b=297';
import {portraitSources,portraitsReady} from './portraits.js?v=297&b=297';
import {BOSS_CATALOG} from './headon-stageboss-patterns.js?v=297&b=297';
...
import{planeSprite,aircraftReady,aircraftKey,hangarArtReady}from './aircraft.js?v=297&b=297';
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale,LEGENDARY_DEFENSE_BALANCE,LEGENDARY_BALANCE}from './engine.js?v=297&b=297';
import {AUGMENTATION_OVERHAUL_BALANCE}from'./augmentation-overhaul150.js?v=293';
import {drawCloudCover}from'./cloud-cover1.js?v=297&b=297';
import {drawFleetLayer}from'./fleet-naval1.js?v=297&b=297';
import {drawTrenchLayer}from'./trench-war1.js?v=297&b=297';
import {drawCityAirLayer}from'./city-air1.js?v=297&b=297';
...
const ententeAirshipSprite=new Image();ententeAirshipSprite.src='./zeppelin-entente.webp?v=297&b=297';
const zeppelinSprite=new Image();zeppelinSprite.src='./zeppelin.webp?v=297&b=297';const ballCloudSprite=new Image();ballCloudSprite.src='./fx-ball-cloud.webp?v=297&b=297';
...
function renderRankingMedals(rows,heading=t('ranking.serverPriority')){
 ...
 const list=document.createElement('ol');list.className='ranking-medal-list';rows.forEach((r,i)=>{const row=document.createElement('li'),badge=document.createElement(i===0?'img':'canvas');badge.className='ranking-medal';
  if(i===0){badge.src='./medal55.webp?v=297&b=297';badge.alt='1 · '+t('ranking.medal.centralDisplay');}else{badge.width=42;badge.height=42;badge.setAttribute('aria-label',(i+1)+' · '+(i===1?t('ranking.medal.ironCross'):t('ranking.medal.ententeDisplay')));drawGameIcon(badge.getContext('2d'),i===1?'ironCross':'victoriaCross',21,21,38);}
...
function fieldArtImg(key){if(!key)return null;let im=fieldArt[key];if(!im){im=new Image();im.src=`./${FIELD_ART_FILE[key]||key}.webp?v=297&b=297`;fieldArt[key]=im}return im.naturalWidth?im:null}
...
const horseEmblem=new Image();horseEmblem.src='./horse-emblem.webp?v=297&b=297';
const xpGem=null;
...
const BOSS_CUTIN_ART={'paris-gun':'boss-art-paris-gun.webp?v=297&b=297','lincomparable':'boss-art-lincomparable.webp?v=297&b=297','sms-stuttgart':'boss-art-sms-stuttgart.webp?v=297&b=297','hms-zubian':'boss-art-hms-zubian.webp?v=297&b=297','a7v-flak':'boss-art-a7v-flak.webp?v=297&b=297','mark-v-cruiser':'boss-art-mark-v-cruiser.webp?v=297&b=297','livens-flame-projector':'boss-art-livens-flame-projector.webp?v=297&b=297','minenwerfer-battery':'boss-art-minenwerfer-battery.webp?v=297&b=297','drachen-net':'boss-art-drachen-net.webp?v=297&b=297','london-apron':'boss-art-london-apron.webp?v=297&b=297','zeppelin-l70':'boss-art-zeppelin-l70.webp?v=297&b=297','hma23':'boss-art-hma23.webp?v=297&b=297','gik':'boss-art-gik.webp?v=297&b=297','ca4':'boss-art-ca4.webp?v=297&b=297','armored-harbor-fortress':'boss-art-armored-harbor-fortress.webp?v=297&b=297'};;
...
function renderCoopSetup(){
 ...
 $('coopPortrait2').src=portraitSources[coopPilot2]||'portrait-'+coopPilot2+'.webp?v=297&b=297';$('coopPortrait2').alt=PILOTS[coopPilot2].name;$('coopPortrait2').style.setProperty('--pilot-choice-scale',choicePortraitScale(coopPilot2));
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='skill'&&e.ownerId){const p=g.player(e.ownerId),id=p.id;coopCutinEnds[id]=g.t+2.1;$(id+'CutinPortrait').src=portraitSources[p.pilot]||'portrait-'+p.pilot+'.webp?v=297&b=297';$(id+'CutinPortrait').alt=PILOTS[p.pilot].name;$(id+'CutinName').textContent=id.toUpperCase()+' · '+p.nickname;$(id+'CutinSkill').textContent=pilotLoadout(p.pilot,p.plane).skill;sfx('skill')}
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;bossCardFrom=performance.now();$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.webp?v=297&b=297';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');bossCutinUntil=bossCardFrom+2600;setBossCutin($('bossPortrait').src,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss')}
