import {drawGrenade,drawGrenadeBlast,drawAmatolBlast,drawFxExplosion} from './weapon-effects156.js?v=326&b=326';
import {fx,fxReady,fxTint,FX56} from './fx-art.js?v=326';
import {CATEGORIES,categoryName,reinforcementName,buildStats,cumulativeText,cleanDescription} from './reinforcement-ui151.js?v=326';
import {t,getLocale,setLocale,subscribe,initLocale,applyTranslations,rarityName,upgradeDescription as translatedUpgradeDescription,pilotName,aircraftName,weaponName,activeName,passiveName,pilotDescription,passiveDescription,aircraftRole,airframeHistory,airframeTip} from './i18n.js?v=326';
import {GamepadInput} from './gamepad-input.js?v=326';
...
import {playerPose,drawPlayerAura,drawPetalParticle,drawRedGhosts162} from './player-effects129.js?v=326&b=326';
import {drawStageBoss,updateStageBossHud,paintCity,paintSky,prepareStageBossAssets} from './stageboss-view.js?v=326&b=326';
import {enableStageBoss,stageBossBounds,harborBankOffset,harborRouteHalfWidth} from './stageboss-host.js?v=326&b=326';
import {chooseTransitionTip,transitionRegionLabel} from './transition-tips188.js?v=326';
import './hud-layout94.js?v=326';
import {showBattlefieldEvent,hideBattlefieldEvent} from './battlefield-event-ui.js?v=326';
import {CoopGame,coopPlane} from './coop-engine.js?v=326&b=326';
import {CoopInput,coopRecord,saveCoopLocal,COOP_RECORD_KEYS} from './coop-input.js?v=326';
import {drawCoop} from './coop-view.js?v=326&b=326';
import {drawSunStrike} from './sun-strike71.js?v=326&b=326';
import {drawEnemyProjectile,drawCannonProjectile,drawBattlefieldFire,friendlyTracerColor} from './projectiles.js?v=326&b=326';
import {installFlightViewport} from './flight-viewport.js?v=326';
import {aircraftFeelRatings,representativeArchetypeKey} from './aircraft-feel174.js?v=326';
import {drawGas} from './gas-view.js?v=326&b=326';
import {missionNavigation,drawMissionRadar} from './navigation.js?v=326&b=326';
import {drawBattlefieldSprite} from './battlefield-art.js?v=326&b=326';
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft,liveryVariant} from './campaign.js?v=326&b=326';
import {drawCampaign} from './campaign-view.js?v=326&b=326';
import {campaignArtReady} from './aircraft.js?v=326&b=326';
import {drawGameIcon,drawSpecialAmmoIcon,iconsReady} from './icons.js?v=326';
import {BattleMusic,musicModeForGame} from './music.js?v=326&b=326';
import {sfx,setSfxMuted} from './sfx.js?v=326&b=326';
import {portraitSources,portraitsReady} from './portraits.js?v=326&b=326';
import {BOSS_CATALOG} from './headon-stageboss-patterns.js?v=326&b=326';
import {drawEquipment} from './equipment.js?v=326&b=326';
import {installHeadOnElitePatch,createEliteAssets,renderEliteLayer} from './elite-patch/module/index.js?v=326';
import{planeSprite,aircraftReady,aircraftKey,hangarArtReady}from './aircraft.js?v=326&b=326';
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale,LEGENDARY_DEFENSE_BALANCE,LEGENDARY_BALANCE}from './engine.js?v=326&b=326';
import {AUGMENTATION_OVERHAUL_BALANCE}from'./augmentation-overhaul150.js?v=326';
import {drawCloudCover}from'./cloud-cover1.js?v=326&b=326';
import {drawFleetLayer}from'./fleet-naval1.js?v=326&b=326';
import {drawTrenchLayer}from'./trench-war1.js?v=326&b=326';
import {drawCityAirLayer}from'./city-air1.js?v=326&b=326';
import {TerrainRenderer,MountainField} from './alps-terrain117.js?v=326';
const flightViewport=installFlightViewport(document,window);
const ententeAirshipSprite=new Image();ententeAirshipSprite.src='./zeppelin-entente.webp?v=326&b=326';
const zeppelinSprite=new Image();zeppelinSprite.src='./zeppelin.webp?v=326&b=326';const ballCloudSprite=new Image();ballCloudSprite.src='./fx-ball-cloud.webp?v=326&b=326';
...
const eliteAssets=createEliteAssets({
 lePrieur:'./elite-patch/assets/le-prieur-squadron.webp?v=326',
 schlachtstaffel:'./elite-patch/assets/halberstadt-cliv-squadron.webp?v=326',
...
function renderRankingMedals(rows,heading=t('ranking.serverPriority')){
 ...
 const list=document.createElement('ol');list.className='ranking-medal-list';rows.forEach((r,i)=>{const row=document.createElement('li'),badge=document.createElement(i===0?'img':'canvas');badge.className='ranking-medal';
  if(i===0){badge.src='./medal55.webp?v=326&b=326';badge.alt='1 · '+t('ranking.medal.centralDisplay');}else{badge.width=42;badge.height=42;badge.setAttribute('aria-label',(i+1)+' · '+(i===1?t('ranking.medal.ironCross'):t('ranking.medal.ententeDisplay')));drawGameIcon(badge.getContext('2d'),i===1?'ironCross':'victoriaCross',21,21,38);}
...
function fieldArtImg(key){if(!key)return null;let im=fieldArt[key];if(!im){im=new Image();im.src=`./${FIELD_ART_FILE[key]||key}.webp?v=326&b=326`;fieldArt[key]=im}return im.naturalWidth?im:null}
...
const horseEmblem=new Image();horseEmblem.src='./horse-emblem.webp?v=326&b=326';
const xpGem=null;
...
const regionTextures={};for(const name of ['sea','trenches']){const im=new Image();im.src='./terrain-'+name+'.webp?v=326';regionTextures[name]=im}
const terrainAlpsAtlas=new Image();terrainAlpsAtlas.src='./terrain-alps-atlas.webp?v=326';
...
const zeeWaterTile=new Image();zeeWaterTile.src='./terrain-zeebrugge-water.webp?v=326';
const zeebruggeHarborArt=new Image();zeebruggeHarborArt.src='./terrain-zeebrugge-route.webp?v=326';
...
const BOSS_CUTIN_ART={'paris-gun':'boss-art-paris-gun.webp?v=326&b=326','lincomparable':'boss-art-lincomparable.webp?v=326&b=326','sms-stuttgart':'boss-art-sms-stuttgart.webp?v=326&b=326','hms-zubian':'boss-art-hms-zubian.webp?v=326&b=326','a7v-flak':'boss-art-a7v-flak.webp?v=326&b=326','mark-v-cruiser':'boss-art-mark-v-cruiser.webp?v=326&b=326','livens-flame-projector':'boss-art-livens-flame-projector.webp?v=326&b=326','minenwerfer-battery':'boss-art-minenwerfer-battery.webp?v=326&b=326','drachen-net':'boss-art-drachen-net.webp?v=326&b=326','london-apron':'boss-art-london-apron.webp?v=326&b=326','zeppelin-l70':'boss-art-zeppelin-l70.webp?v=326&b=326','hma23':'boss-art-hma23.webp?v=326&b=326','gik':'boss-art-gik.webp?v=326&b=326','ca4':'boss-art-ca4.webp?v=326&b=326','armored-harbor-fortress':'boss-art-armored-harbor-fortress.webp?v=326&b=326'};;
...
function renderCoopSetup(){
 ...
 $('coopPortrait2').src=portraitSources[coopPilot2]||'portrait-'+coopPilot2+'.webp?v=326&b=326';$('coopPortrait2').alt=PILOTS[coopPilot2].name;$('coopPortrait2').style.setProperty('--pilot-choice-scale',choicePortraitScale(coopPilot2));
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='skill'&&e.ownerId){const p=g.player(e.ownerId),id=p.id;coopCutinEnds[id]=g.t+2.1;$(id+'CutinPortrait').src=portraitSources[p.pilot]||'portrait-'+p.pilot+'.webp?v=326&b=326';$(id+'CutinPortrait').alt=PILOTS[p.pilot].name;$(id+'CutinName').textContent=id.toUpperCase()+' · '+p.nickname;$(id+'CutinSkill').textContent=pilotLoadout(p.pilot,p.plane).skill;sfx('skill')}
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;bossCardFrom=performance.now();$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.webp?v=326&b=326';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');bossCutinUntil=bossCardFrom+2600;setBossCutin($('bossPortrait').src,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss')}
...
const rankingMedalImages=Object.fromEntries(['central','entente'].flatMap(side=>[1,2,3].map(rank=>{
 const image=new Image();image.src=`./ranking-${side}-${rank}.webp?v=326`;return [`${side}-${rank}`,image];
})));
const aircraftIronCrossImage=new Image();aircraftIronCrossImage.src='./aircraft-iron-cross.svg?v=326';
const aircraftRoundelImage=new Image();aircraftRoundelImage.src='./aircraft-roundel.svg?v=326';
