import {MobilePerformanceClock,readPerformanceOptions,performanceCanvasSize} from './mobile-performance.js?v=thermal-20260926-r1';
...
import {t,getLocale,setLocale,subscribe,initLocale,applyTranslations,rarityName,upgradeDescription as translatedUpgradeDescription,pilotName,aircraftName,weaponName,activeName,passiveName,pilotDescription,passiveDescription,aircraftRole,airframeHistory,airframeTip} from './i18n.js?v=340';
...
import {CoopGame,coopPlane} from './coop-engine.js?v=340&b=340&thermal=20260926r1';
import {CoopInput,coopRecord,saveCoopLocal,COOP_RECORD_KEYS} from './coop-input.js?v=340';
import {drawCoop} from './coop-view.js?v=340&b=340&thermal=20260926r1';
import {drawSunStrike} from './sun-strike71.js?v=340&b=340&thermal=20260926r1';
import {drawEnemyProjectile,drawCannonProjectile,drawBattlefieldFire,friendlyTracerColor} from './projectiles.js?v=340&b=340';
import {installFlightViewport} from './flight-viewport.js?v=340';
import {aircraftFeelRatings,representativeArchetypeKey} from './aircraft-feel174.js?v=340';
import {drawGas} from './gas-view.js?v=340&b=340';
import {missionNavigation,drawMissionRadar} from './navigation.js?v=340&b=340';
import {drawBattlefieldSprite} from './battlefield-art.js?v=340&b=340';
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft,liveryVariant} from './campaign.js?v=340&b=340&thermal=20260926r1';
import {drawCampaign} from './campaign-view.js?v=340&b=340&thermal=20260926r1';
import {campaignArtReady} from './aircraft.js?v=340&b=340';
import {drawGameIcon,drawSpecialAmmoIcon,iconsReady} from './icons.js?v=340';
import {BattleMusic,musicModeForGame} from './music.js?v=340&b=340';
import {sfx,setSfxMuted,setSfxPaused,stopSfx} from './sfx.js?v=340';
import {drawHeadOnFeedback} from './engagement-feedback.js?v=340';
import {portraitSources,portraitsReady} from './portraits.js?v=340&b=340';
import {BOSS_CATALOG} from './headon-stageboss-patterns.js?v=340&b=340';
import {bossTactic,bossPhaseLabel,BOSS_NAMES_EN} from './boss-feedback.js?v=340';
import {drawEquipment} from './equipment.js?v=340&b=340';
import {installHeadOnElitePatch,createEliteAssets,renderEliteLayer} from './elite-patch/module/index.js?v=340';
import{planeSprite,aircraftReady,aircraftKey,hangarArtReady}from './aircraft.js?v=340&b=340';
import{Game,PLANES,PILOTS,UPGRADES,WEAPONS,PILOT_PLANES,upgradeDescription,pilotLoadout,pilotAircraftName,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale,LEGENDARY_DEFENSE_BALANCE,LEGENDARY_BALANCE}from './engine.js?v=340&b=340&thermal=20260926r1';
import {AUGMENTATION_OVERHAUL_BALANCE}from'./augmentation-overhaul150.js?v=340';
import {drawCloudCover}from'./cloud-cover1.js?v=340&b=340&thermal=20260926r1';
import {drawFleetLayer}from'./fleet-naval1.js?v=340&b=340&thermal=20260926r1';
import {drawTrenchLayer}from'./trench-war1.js?v=340&b=340&thermal=20260926r1';
import {drawCityAirLayer}from'./city-air1.js?v=340&b=340&thermal=20260926r1';
import {TerrainRenderer,MountainField} from './alps-terrain117.js?v=340';
const flightViewport=installFlightViewport(document,window);
const ententeAirshipSprite=new Image();ententeAirshipSprite.src='./zeppelin-entente.webp?v=340&b=340';
const zeppelinSprite=new Image();zeppelinSprite.src='./zeppelin.webp?v=340&b=340';const ballCloudSprite=new Image();ballCloudSprite.src='./fx-ball-cloud.webp?v=340&b=340';
...
const eliteAssets=createEliteAssets({
 lePrieur:'./elite-patch/assets/le-prieur-squadron.webp?v=340',
 schlachtstaffel:'./elite-patch/assets/halberstadt-cliv-squadron.webp?v=340',
...
const mobilePerformanceOptions=readPerformanceOptions(globalThis.location?.search||'',window.matchMedia('(pointer:coarse), (max-width:720px)').matches);
const performanceClock=new MobilePerformanceClock(mobilePerformanceOptions);
let frameRequestId=null,performanceWork=0,performanceLastDraw=null;
...
function events(){for(let e of game.events.splice(0)){if(e.type==='bossPhase'){$('toast').textContent=bossPhaseLabel(e.text,getLocale());show('toast');toastUntil=performance.now()+2200;continue}if(e.type==='bossSound'){sfx(e.text);continue}if(e.type==='regionTransition')beginRegionTransition(e.text,e.region);if(e.type==='bossArrival'){bossArrivalUntil=game.t+4.5;$('bossName').textContent=e.pilot?pilotName(e.pilot,e.name||t('warning.ace')):(e.name||t('warning.ace'));$('bossPortrait').src=portraitSources[e.pilot]||TRANSPARENT_PORTRAIT;$('bossPortrait').alt=$('bossName').textContent;$('bossArrivalDetail').textContent=e.pilot?(PILOTS[e.pilot]?.alias||''):t('warning.airspace');$('bossWarnSub').textContent=$('bossName').textContent+' · ENEMY ACE INBOUND';bossCardFrom=performance.now();bossCutinUntil=bossCardFrom+2600;const aceImg=portraitSources[e.pilot]||(e.pilot?'portrait-'+e.pilot+'.webp?v=340&b=340':(e.airframe?'mech/'+e.airframe+'.webp?v=340&b=340':''));setBossCutin(aceImg,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss');sfx('aceSting');{const arrivalRun=game;setTimeout(()=>{if(game===arrivalRun&&game.state==='playing'&&!regionTransitionUntil&&!document.hidden)sfx('aceSting')},380)}}if(e.type==='upgrade'){choices=game.rollChoices().map(u=>({id:u.id,rarity:u.rarity}));const rarityName={normal:'일반',magic:'매직',rare:'레어',unique:'유니크',legendary:'특수장비'};modal('LEVEL '+game.level+' / FIELD UPGRADE','강화 선택','전투에 적용할 강화를 하나 선택하세요',choices.map((pick,i)=>{let u=UPGRADES.find(u=>u.id===pick.id);return{upgradeId:pick.id,label:`${i+1}. [${rarityName[pick.rarity]}] ${u.name}`,rarity:pick.rarity,desc:upgradeDescription(pick.id,pick.rarity,game),run:()=>choose(pick.id,pick.rarity)}}));sfx('levelup')}if(['wave','skill','ally','flak'].includes(e.type)){$('toast').textContent=runtimeEventText(e.text);show('toast');toastUntil=performance.now()+2200;if(e.type==='skill'){sfx('skill');const p=PILOTS[pilot]||{};setBossCutin(portraitSources[pilot]||'portrait-'+pilot+'.webp?v=340&b=340',p.skill||'액티브','',p.name||pilot,'skill');bossCardFrom=performance.now();bossCutinUntil=bossCardFrom+2100}else sfx(e.type==='flak'?'flak':e.type==='ally'?'ally':'wave')}if(e.type==='reload')sfx('reload');if(e.type==='loaded')sfx('loaded');if(e.type==='shot'&&performance.now()-lastShotSound>100){lastShotSound=performance.now();sfx('shot')}if(e.type==='impact'&&performance.now()-lastImpactSound>65){lastImpactSound=performance.now();sfx('impact')}if(e.type==='pickup'&&performance.now()-lastPickupSound>220){lastPickupSound=performance.now();sfx(e.text==='heal'?'heal':'pickup')}if(e.type==='hit')sfx('hit');if(e.type==='kill')sfx(e.text==='balloon'?'balloon':'kill');if(e.type==='end')sfx(game.state==='won'||game.result?.won?'victory':'defeat');if(e.type==='end'&&game.mode==='campaign'){campaignEnd(game);continue}if(e.type==='end'){showSoloResult(game)}}}
...
function drawPilotPassives(g,x,y,t,point){
 ...
 if(pil===`jacobs`&&(pl.jacobsStacks||0)>0){ctx.save();for(let i=0;i<pl.jacobsStacks;i++){const ga=t*2.4+i*Math.PI*2/3+.6,ox=x+Math.cos(ga)*34,oy=y+Math.sin(ga)*34+Math.sin(t*6+i)*2;fxGlow(ox,oy,11,`rgba(8,4,2,.55)`);ctx.globalAlpha=.94;ctx.translate(ox,oy);ctx.rotate(ga+Math.PI/2);ctx.fillStyle=`#1c1410`;ctx.beginPath();ctx.arc(0,-2,4.4,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.moveTo(-3.6,-4.5);ctx.lineTo(-6.5,-10.5);ctx.lineTo(-1,-6.5);ctx.closePath();ctx.moveTo(3.6,-4.5);ctx.lineTo(6.5,-10.5);ctx.lineTo(1,-6.5);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(-3.4,1.5);ctx.quadraticCurveTo(-7.5,5,-5,9);ctx.lineTo(0,6.5);ctx.lineTo(5,9);ctx.quadraticCurveTo(7.5,5,3.4,1.5);ctx.closePath();ctx.fill();fxSpark(0,-2.5,5,`#ff4a3a`,.9,t*4+i);resetGameTransform()}ctx.restore()}
...
function draw(t){let cx=game?game.x:Math.sin(t*.03)*45,cy=game?game.y:-t*12;ctx.save();if(game?.shake)ctx.translate((Math.random()-.5)*game.shake,(Math.random()-.5)*game.shake);terrain(cx,cy);drawStageBoss(ctx,game,W,H,{drawZeppelin,drawFieldArt,layer:'bodies'});if(game){ctx.strokeStyle='#ecf6de35';ctx.lineWidth=1;for(let i=0;i<16;i++){let px=(i*137.3+t*73)%W,py=(i*97.7+t*170)%H;ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px-Math.cos(game.a)*22,py-Math.sin(game.a)*22);ctx.stroke()}}const point=(x,y)=>[x-cx+W/2,y-cy+H/2];if(game){drawFleetLayer(ctx,game,{point});drawTrenchLayer(ctx,game,{point});drawCityAirLayer(ctx,game,{point})}if(game){
 ...
 for(const g of game.dreideckerGhosts||[]){const[gx,gy]=point(g.x,g.y),st=g.stretch?1.5:1;ctx.save();ctx.globalAlpha=(.15+.22*(g.turn||0))*(g.life/g.maxLife)*(g.stretch?1.3:1);ctx.filter='brightness(.5)';planeSprite(ctx,gx-Math.cos(g.a)*18*(st-1),gy-Math.sin(g.a)*18*(st-1),g.a,aircraftKey(plane,false,pilot),1,false,false);ctx.restore()}
...
function readSoloFrameInput(){
 ...
 const controller=gamepadInput.poll(true);let angle;
 if(joy!=null){gamepadInput.useTouch();controller.inputMode='touch';angle=joy}
 else if(dx||dy){gamepadInput.useKeyboard();controller.inputMode='keyboard';angle=Math.atan2(dy,dx)}
 else if(controller.inputMode==='gamepad'&&(controller.moveX||controller.moveY))angle=Math.atan2(controller.moveY,controller.moveX);
 if(controller.inputMode==='gamepad'){
  if(controller.reloadJustPressed)game.reload();if(controller.activeJustPressed)game.skill();
  if(controller.maneuverJustPressed)game.evade();if(controller.pauseJustPressed)pause();
 }
 return {angle,moveX:controller.moveX,moveY:controller.moveY,fireHeld:controller.inputMode==='gamepad'?controller.fireHeld:undefined,inputMode:controller.inputMode};
}
function renderPerformanceFrame(now){
 resetGameTransform();
 if(game?.mode==='coop2'){
  coopHud();relicCooldownTick();
  drawCoop(ctx,game,W,H,{terrain:(x,y,w,h)=>paintRegion(game.worldRegion(),x,y,w,h),drawZeppelin,drawFieldArt,fieldArt,fieldArtImg,groundLayer:()=>{const wp=(x,y)=>[x,y];drawFleetLayer(ctx,game,{point:wp});drawTrenchLayer(ctx,game,{point:wp});drawCityAirLayer(ctx,game,{point:wp})}});
  drawLegendaryDefenseOverlay(game,ctx,W,H);drawStageBoss(ctx,game,W,H,{drawZeppelin,drawFieldArt,layer:'hazards'});
  const z=game.camera?.zoom||1;drawCloudCover(ctx,game,{point:(x,y)=>[(x-game.x)*z+W/2,(y-game.y)*z+H/2],scale:z,region:game.worldRegion?.()});
  updateStageBossHud(game);drawHeadOnFeedback(ctx,game,W,H,sfx);stageBossCutinCheck();
  show('bossCutin',game.state==='playing'&&performance.now()>=bossCardFrom&&performance.now()<bossCutinUntil);return;
 }
 if(game)hud();stageBossCutinCheck();
 show('bossCutin',!!game&&game.state==='playing'&&performance.now()>=bossCardFrom&&performance.now()<bossCutinUntil);
 draw(ambient);drawLegendaryDefenseOverlay(game,ctx,W,H);drawSupport();drawCampaign(ctx,game,W,H);
 drawSunStrike(ctx,game,W,H);drawStageBoss(ctx,game,W,H,{drawZeppelin,drawFieldArt,layer:'hazards'});
 if(game)drawCloudCover(ctx,game,{point:(x,y)=>[x-game.x+W/2,y-game.y+H/2],scale:1,region:game.worldRegion?.()});
 updateStageBossHud(game);drawHeadOnFeedback(ctx,game,W,H,sfx);
 if(game?.mode==='campaign')campaignHud();if(now>toastUntil)show('toast',false);
 if(now>cutinUntil||!game||['lost','won'].includes(game.state))show('skillCutin',false);
}
function frame(now){
 frameRequestId=null;
 if(!mobilePerformanceOptions.clock)return legacyFrame(now);
 const workStart=performance.now();let rendered=false,running=false;
 try{
  if(document.hidden){performanceClock.reset(now);return;}
  if(regionTransitionUntil&&now>=regionTransitionUntil&&(regionTransitionAssetsReady||now>=regionTransitionUntil+6000)){
   regionTransitionUntil=0;show('regionTransition',false);performanceClock.reset(now);
  }
  const transitioning=!!game&&regionTransitionUntil>0;
  setSfxPaused(transitioning||game?.state==='paused');
  if(transitioning){performanceClock.reset(now);performanceWork=0;performanceLastDraw=null;return;}
  const session=game;
  const input=game?.mode==='coop2'?(gamepadInput.poll(false),coopInput.inputs()):game?readSoloFrameInput():{};
  running=game?.state==='playing';
  const steps=performanceClock.advance(now,running);
  if(game){game.viewWidth=W;game.viewHeight=H;game.useCollisionGrid=mobilePerformanceOptions.grid;game.visualParticleLimit=performanceClock.particleLimit();}
  for(let i=0;i<steps;i++){
   if(!game||game!==session||game.state!=='playing'||regionTransitionUntil){performanceClock.discard();break;}
   game.update(performanceClock.step,input);performanceClock.simulationSteps++;ambient+=performanceClock.step;
   if(game.mode==='coop2')coopEvents();else events();
  }
  if(regionTransitionUntil)return;
  if(!game)ambient+=Math.min(.04,Math.max(0,(now-last)/1000||0));last=now;
  if(performanceClock.changed){performanceClock.changed=false;resize();}
  if(performanceClock.shouldRender(now,game?.state||'hangar')){renderPerformanceFrame(now);rendered=true;}
  frameErrors=0;
 }catch(error){console.error('[headon]',error);if(++frameErrors>20)reportFrameFatal(error);}
 finally{
  if(running){
   performanceWork+=Math.max(0,performance.now()-workStart);
   if(rendered){performanceClock.sample(now,performanceWork,performanceLastDraw===null?0:now-performanceLastDraw);performanceWork=0;performanceLastDraw=now;}
  }else{performanceWork=0;performanceLastDraw=null;performanceClock.sampleStart=null;performanceClock.sampleWork=performanceClock.sampleCount=performanceClock.sampleLate=0;}
  if(!frameFatalShown)queueFrame();
 }
}
document.addEventListener('visibilitychange',()=>{
 if(!mobilePerformanceOptions.clock)return;
 performanceClock.reset();
 if(document.hidden){if(frameRequestId!=null)cancelAnimationFrame(frameRequestId);frameRequestId=null;}
 else queueFrame();
});
if(mobilePerformanceOptions.debug)Object.defineProperty(globalThis,'__headonPerformance',{configurable:true,value:Object.freeze({
 snapshot:()=>({...performanceClock.snapshot(),worldWidth:W,worldHeight:H,backingWidth:canvas.width,backingHeight:canvas.height,
  gameTime:game?.t||0,roundsFired:game?.roundsFired||0,particleCount:game?.particles?.length||0,
  enemyCount:game?.enemies?.length||0,bulletCount:game?.bullets?.length||0,grid:game?._projectileGrid?.stats()||null,state:game?.state||'hangar',frameErrors}),
})});
roster();resize();queueFrame();
...
function renderRankingMedals(rows,heading=t('ranking.serverPriority')){
 ...
 const list=document.createElement('ol');list.className='ranking-medal-list';rows.forEach((r,i)=>{const row=document.createElement('li'),badge=document.createElement(i===0?'img':'canvas');badge.className='ranking-medal';
  if(i===0){badge.src='./medal55.webp?v=340&b=340';badge.alt='1 · '+t('ranking.medal.centralDisplay');}else{badge.width=42;badge.height=42;badge.setAttribute('aria-label',(i+1)+' · '+(i===1?t('ranking.medal.ironCross'):t('ranking.medal.ententeDisplay')));drawGameIcon(badge.getContext('2d'),i===1?'ironCross':'victoriaCross',21,21,38);}
...
syncServerRanking=async()=>{const run=game;if(game?.mode==='campaign'||game?.mode==='coop2')return;try{const res=await fetch('/api/rankings',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:nickname,score:game.recordedKills?game.recordedKills():(game.priorityKills||0),pilot:PILOTS[pilot].name,season:'priority-340'})});if(!res.ok)return;const rows=await res.json();if(game!==run)return;renderRankingMedals(rows)}catch{}};


const fieldArt={};const FIELD_ART_FILE={flak:'flak-burst',gust:'gust',stork:'portrait-stork'};
function fieldArtImg(key){if(!key)return null;let im=fieldArt[key];if(!im){im=new Image();im.src=`./${FIELD_ART_FILE[key]||key}.webp?v=340&b=340`;fieldArt[key]=im}return im.naturalWidth?im:null}
for(const key of Object.keys(FIELD_ART_FILE))fieldArtImg(key);
const fieldRim={};
...
const horseEmblem=new Image();horseEmblem.src='./horse-emblem.webp?v=340&b=340';
const xpGem=null;
...
const regionTextures={};for(const name of ['sea','trenches']){const im=new Image();im.src='./terrain-'+name+'.webp?v=340';regionTextures[name]=im}
const terrainAlpsAtlas=new Image();terrainAlpsAtlas.src='./terrain-alps-atlas.webp?v=340';
...
const zeeWaterTile=new Image();zeeWaterTile.src='./terrain-zeebrugge-water.webp?v=340';
const zeebruggeHarborArt=new Image();zeebruggeHarborArt.src='./terrain-zeebrugge-route.webp?v=340';
...
const terrainKeys=['rural','sea','trenches','burning','city','sky','alps','zeebrugge','cambrai'];
...
draw=t=>{
 ...
 ctx.save();ctx.font='12px monospace';ctx.textAlign='left';ctx.fillStyle='#f1edd0';ctx.fillText((game.mode==='campaign'?game.stage.region:['전원 지대','아드리아해','참호 전선','포화의 참호전선','도심','고공 전역','알프스 산맥','제브뤼헤 군항','캉브레 들판'][game.region||0])+' · 비행 '+((game.distance||0)/1000).toFixed(1)+' km',14,H-14);
...
const BOSS_CUTIN_ART={'paris-gun':'boss-art-paris-gun.webp?v=340&b=340','lincomparable':'boss-art-lincomparable.webp?v=340&b=340','sms-stuttgart':'boss-art-sms-stuttgart.webp?v=340&b=340','hms-zubian':'boss-art-hms-zubian.webp?v=340&b=340','a7v-flak':'boss-art-a7v-flak.webp?v=340&b=340','mark-v-cruiser':'boss-art-mark-v-cruiser.webp?v=340&b=340','livens-flame-projector':'boss-art-livens-flame-projector.webp?v=340&b=340','minenwerfer-battery':'boss-art-minenwerfer-battery.webp?v=340&b=340','drachen-net':'boss-art-drachen-net.webp?v=340&b=340','london-apron':'boss-art-london-apron.webp?v=340&b=340','zeppelin-l70':'boss-art-zeppelin-l70.webp?v=340&b=340','hma23':'boss-art-hma23.webp?v=340&b=340','gik':'boss-art-gik.webp?v=340&b=340','ca4':'boss-art-ca4.webp?v=340&b=340','armored-harbor-fortress':'boss-art-armored-harbor-fortress.webp?v=340&b=340','fliegerzug':'boss-art-rail-carrier.webp?v=340&b=340','treffas-wagen':'boss-art-treffas-wagen.webp?v=340&b=340'};;
...
function renderCoopSetup(){
 ...
 $('coopPortrait2').src=portraitSources[coopPilot2]||'portrait-'+coopPilot2+'.webp?v=340&b=340';$('coopPortrait2').alt=PILOTS[coopPilot2].name;$('coopPortrait2').style.setProperty('--pilot-choice-scale',choicePortraitScale(coopPilot2));
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='skill'&&e.ownerId){const p=g.player(e.ownerId),id=p.id;coopCutinEnds[id]=g.t+2.1;$(id+'CutinPortrait').src=portraitSources[p.pilot]||'portrait-'+p.pilot+'.webp?v=340&b=340';$(id+'CutinPortrait').alt=PILOTS[p.pilot].name;$(id+'CutinName').textContent=id.toUpperCase()+' · '+p.nickname;$(id+'CutinSkill').textContent=pilotLoadout(p.pilot,p.plane).skill;sfx('skill')}
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;bossCardFrom=performance.now();$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.webp?v=340&b=340';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');bossCutinUntil=bossCardFrom+2600;setBossCutin($('bossPortrait').src,$('bossName').textContent,'ENEMY ACE · 적 에이스',$('bossArrivalDetail').textContent);setBgmMode('boss')}
...
const rankingMedalImages=Object.fromEntries(['central','entente'].flatMap(side=>[1,2,3].map(rank=>{
 const image=new Image();image.src=`./ranking-${side}-${rank}.webp?v=340`;return [`${side}-${rank}`,image];
})));
const aircraftIronCrossImage=new Image();aircraftIronCrossImage.src='./aircraft-iron-cross.svg?v=340';
const aircraftRoundelImage=new Image();aircraftRoundelImage.src='./aircraft-roundel.svg?v=340';
...
const HEADON_TEST_REGION_NAMES=['전원 지대','아드리아해','참호 전선','포화의 참호전선','도심','고공 전역','알프스 산맥','제브뤼헤 군항','캉브레 들판'];
