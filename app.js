...
import {drawSunStrike} from './sun-strike71.js?v=141';
import {drawEnemyProjectile,drawCannonProjectile,drawBattlefieldFire} from './projectiles.js?v=135';
...
import {drawGas} from './gas-view.js?v=201';
import {missionNavigation,drawMissionRadar} from './navigation.js?v=128';
import {drawBattlefieldSprite} from './battlefield-art.js?v=201';
import {CampaignGame,STAGES,stageFaction,historicalAircraft,sortieAircraft} from './campaign.js?v=174';
import {drawCampaign} from './campaign-view.js?v=128';
import {campaignArtReady} from './aircraft.js?v=184';
import {drawGameIcon,drawSpecialAmmoIcon,iconsReady} from './icons.js?v=158';
import {BattleMusic,musicModeForGame} from './music.js?v=201';
import {sfx,setSfxMuted} from './sfx.js?v=185';
import {portraitSources,portraitsReady} from './portraits.js?v=145';
import {drawEquipment} from './equipment.js?v=201';
...
const ententeAirshipSprite=new Image();ententeAirshipSprite.src='./zeppelin-entente.png?v=201&b=201';
const zeppelinSprite=new Image();zeppelinSprite.src='./zeppelin.png?v=201&b=201';
...
function resize(){let r=canvas.getBoundingClientRect();const pixelScale=window.matchMedia('(max-width:720px), (pointer:coarse) and (max-height:600px)').matches?.8:1.25;W=Math.max(240,Math.round(r.width/pixelScale));H=Math.max(240,Math.round(r.height/pixelScale));canvas.width=W;canvas.height=H;ctx.imageSmoothingEnabled=false}new ResizeObserver(resize).observe($('viewport'));
...
function renderRankingMedals(rows,heading=t('ranking.serverPriority')){
 ...
 const list=document.createElement('ol');list.className='ranking-medal-list';rows.forEach((r,i)=>{const row=document.createElement('li'),badge=document.createElement(i===0?'img':'canvas');badge.className='ranking-medal';
  if(i===0){badge.src='./medal55.png?v=201&b=201';badge.alt='1 · '+t('ranking.medal.centralDisplay');}else{badge.width=42;badge.height=42;badge.setAttribute('aria-label',(i+1)+' · '+(i===1?t('ranking.medal.ironCross'):t('ranking.medal.ententeDisplay')));drawGameIcon(badge.getContext('2d'),i===1?'ironCross':'victoriaCross',21,21,38);}
...
const fieldArt={};for(const [key,file] of Object.entries({flak:'fx-flak',gust:'gust',stork:'portrait-stork',staaken:'staaken','handley-page':'handley-page'})){const im=new Image();im.src=`./${file}.png?v=201&b=201`;fieldArt[key]=im}
...
const horseEmblem=new Image();horseEmblem.src='./horse-emblem.png?v=201&b=201';
...
function renderCoopSetup(){
 ...
 $('coopPortrait2').src=portraitSources[coopPilot2]||'portrait-'+coopPilot2+'.png?v=201&b=201';$('coopPortrait2').alt=PILOTS[coopPilot2].name;$('coopPortrait2').style.setProperty('--pilot-choice-scale',choicePortraitScale(coopPilot2));
...
function coopEvents(){
 ...
 for(const e of g.events.splice(0)){
  ...
  if(e.type==='skill'&&e.ownerId){const p=g.player(e.ownerId),id=p.id;coopCutinEnds[id]=g.t+2.1;$(id+'CutinPortrait').src=portraitSources[p.pilot]||'portrait-'+p.pilot+'.png?v=201&b=201';$(id+'CutinPortrait').alt=PILOTS[p.pilot].name;$(id+'CutinName').textContent=id.toUpperCase()+' · '+p.nickname;$(id+'CutinSkill').textContent=pilotLoadout(p.pilot,p.plane).skill;sfx('skill')}
  if(e.type==='bossArrival'){bossArrivalUntil=g.t+4.5;$('bossName').textContent=arrivalCount>1?t('warning.aceGroup'):e.name;$('bossPortrait').src=portraitSources[e.pilot]||'portrait-'+e.pilot+'.png?v=201&b=201';$('bossArrivalDetail').textContent=arrivalCount>1?t('warning.aceApproach',{count:arrivalCount}):t('coop.aceCoop');setBgmMode('boss')}
...
async function showSoloLeaderboard(){if(game||selectedMode!=='endless')return;modal('RANKING MODE',t('ranking.title'),t('ranking.loading'),[{label:t('sortie.screen'),run:()=>show('modal',false)}]);$('modal').classList.add('ranking-modal190');try{const response=await fetch('/api/rankings');if(!response.ok)throw Error();const rows=await response.json();if(!game&&!$('modal').classList.contains('hidden')&&$('modalTag').textContent==='RANKING MODE')renderFactionRanking(rows)}catch{if(!game&&!$('modal').classList.contains('hidden')&&$('modalTag').textContent==='RANKING MODE')$('modalText').textContent=t('ranking.loadFailed')}}
...
function showBuildPause151(){
 modal('PAUSED','','',[{label:'계속하기',run:resume},{label:'설정',run:showSettings151},{label:'전투 종료',run:returnHangar}]);$('modal').classList.add('build-modal151');
