...
import {SUN_STRIKE,TAILING_BALANCE,SPECIAL_AMMO,enemyAircraftScale} from './engine.js?v=201';
...
export function drawCoop(c,g,W,H,{terrain,drawZeppelin,drawFieldArt,fieldArt}){
 ...
 for(const particle of g.particles){const smokeLife=particle.smoke?Math.max(0,particle.life/particle.maxLife):0;c.globalAlpha=particle.smoke?smokeLife*(particle.muzzleSmoke?.88:.65):Math.min(1,particle.life*3);if(particle.hitFx){c.globalAlpha=Math.min(1,particle.life*8);fxCell(c,'mgTiers',MG_TIER_COLUMN[particle.mgTier||'NORMAL']??0,particle.hitFx==='armor'?MG_TIER_ROW.hit_armor:MG_TIER_ROW.hit_air,64,64,particle.x,particle.y,32,32);c.globalAlpha=1;continue}if(particle.petal){drawPetalParticle(c,particle,particle.x,particle.y);continue}if(particle.heart){heart(particle.x,particle.y,particle.size,particle.color);continue}c.fillStyle=particle.color;const size=particle.smoke?particle.size+(1-smokeLife)*(particle.muzzleSmoke?15:10):3;
 ...
 for(const p of g.players){if(p.status!=='alive')continue;
  ...
  c.globalAlpha=p.invuln>0&&isFinite(p.invuln)&&Math.floor(t*15)%2?.55:1;sprite(p,aircraftKey(p.plane,false,p.pilot));c.globalAlpha=1;const color=p.id==='p1'?'#74d9fb':'#ffcd78';ring(p.x,p.y,39,color,1.5);c.fillStyle=color;c.textAlign='center';c.font='bold 14px sans-serif';c.fillText(p.id.toUpperCase(),p.x,p.y+54);
 if(p.muzzleFlash>0&&p.reloadTime===0)for(let gun=0;gun<p.weapon.guns;gun++){const ga=p.gunDirection(gun),mx=p.x+Math.cos(ga)*27,my=p.y+Math.sin(ga)*27;if(!fxCell(c,'mgTiers',MG_TIER_COLUMN[g.mgFxTier?g.mgFxTier():'NORMAL']??0,MG_TIER_ROW.muzzle,64,64,mx+Math.cos(ga)*6,my+Math.sin(ga)*6,18,18,ga)){c.fillStyle='#fff4ca';c.fillRect(mx-2,my-2,5,5)}}
 }
