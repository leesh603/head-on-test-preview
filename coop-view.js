...
export function drawCoop(c,g,W,H,{terrain,drawZeppelin,drawFieldArt,fieldArt}){
 ...
 for(const particle of g.particles){const smokeLife=particle.smoke?Math.max(0,particle.life/particle.maxLife):0;c.globalAlpha=particle.smoke?smokeLife*(particle.muzzleSmoke?.88:.65):Math.min(1,particle.life*3);if(particle.hitFx){c.globalAlpha=Math.min(1,particle.life*8);fxCell(c,'mgTiers',MG_TIER_COLUMN[particle.mgTier||'NORMAL']??0,particle.hitFx==='armor'?MG_TIER_ROW.hit_armor:MG_TIER_ROW.hit_air,64,64,particle.x,particle.y,26,26);c.globalAlpha=1;continue}if(particle.petal){drawPetalParticle(c,particle,particle.x,particle.y);continue}if(particle.heart){heart(particle.x,particle.y,particle.size,particle.color);continue}c.fillStyle=particle.color;const size=particle.smoke?particle.size+(1-smokeLife)*(particle.muzzleSmoke?15:10):3;
