...
import {aircraftKey,aircraftPreviewURL} from './aircraft.js?v=190';
...
function install(){
 ...
 function sync(){
  ...
  put(title,pilot==='baron'?(en?'The Red Baron':'붉은 남작'):$('hangarName').textContent);
  put(english,pilotEnglishNames[pilot]||$('pilotAlias').textContent);
  ...
  for(const button of $('pilotTabs').children){
   const id=button.dataset.pilotId,label=button.querySelector('.pilot-tab-label');
   if(id==='baron')put(label,en?'The Red Baron':'붉은 남작');
   button.title=pilotEnglishNames[id]||label?.textContent||'';
   let caption=button.querySelector('.astra-pilot-caption');if(!caption){caption=el('small','astra-pilot-caption');button.append(caption)}put(caption,pilotEnglishNames[id]||'');
  }
