...
export function drawRailTrack(c,boss,image){
 ...
 const end=(boss.railTarget.x-boss.from.x)*boss.axis.x+(boss.railTarget.y-boss.from.y)*boss.axis.y,stop=end+50;
 for(let y=-270;y<stop;y+=19){if(boss.broken&&Math.abs(y-end)<27)continue;timber(c,image,0,y);}
 for(let y=-270;y<stop;y+=42){
