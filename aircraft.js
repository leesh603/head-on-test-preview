...
const individualAircraftReady=INDIVIDUAL_KEYS.map(key=>new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);const sourceKey=key==='nieuport_italian'?'nieuport':key;img.src=new URL(`./${sourceKey}.png?v=191`,import.meta.url).href;
}));
...
const damageDecalReady=new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);img.src=new URL('./damage-decals.png?v=191',import.meta.url).href;
});
...
export function aircraftPreviewURL(key){
 ...
 const p=new Promise(resolve=>{
  ...
  img.src=new URL(`./${fileKey}.png?v=191`,import.meta.url).href;
 });
...
export const campaignArtReady=new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);img.src=new URL('./campaign-units.png?v=191',import.meta.url).href;
});
