import {clearCrewMatte} from './matte70.js?v=325&b=325';
...
const loadPainted=key=>new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);const sourceKey=key==='nieuport_italian'?'nieuport':key;const mechDir=new URLSearchParams(location.search).get('mech')==='0'?'':'mech/';img.src=new URL(`./${mechDir}${sourceKey}.webp?v=325&b=325`,import.meta.url).href;
});
...
const newAceAircraftReady=!NEW_ACE_AIRCRAFT.length?Promise.resolve(true):new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);img.src=new URL('./new-aces-aircraft124.webp?v=325&b=325',import.meta.url).href;
});
...
const damageDecalReady=new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);img.src=new URL('./damage-decals.webp?v=325&b=325',import.meta.url).href;
});
...
export const campaignArtReady=new Promise(resolve=>{
 ...
 };img.onerror=()=>resolve(false);img.src=new URL('./campaign-units.webp?v=325&b=325',import.meta.url).href;
});
