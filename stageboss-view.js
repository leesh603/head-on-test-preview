import {drawRailDamage,drawRailTrack} from './rail-render129.js?v=305&b=305';
import {fx,fxReady,fxImage} from './fx-art.js?v=305';
import {drawEnemyProjectile} from './projectiles.js?v=305&b=305';
import {drawSupportShip,drawSupportEffects} from './stuttgart-render129.js?v=305';
import {renderStageBossLayer} from './headon-stageboss-render.js?v=305';
import {bossHudModel} from './headon-stageboss-hud.js?v=305&b=305';
...
const bossSources={
 parisGun:'./boss-bruno-train115.webp',lincomparable:'./boss-lincomparable94.webp?v=305&b=305',stuttgart:'./boss-sms-stuttgart94.webp',zubian:'./boss-hms-zubian94.webp',
 l70:'./boss-zeppelin-l7094.webp',hma23:'./boss-hma2394.webp',a7v:'./boss-a7v-flak94.webp',markv:'./boss-mark-v94.webp',gik:'./boss-gik.webp?v=305&b=305',ca4:'./boss-ca4.webp?v=305&b=305',
...
const railWreckSources={
 parisGun:{engine:'./rail-boss-bruno-engine-wreck192.webp?v=305&b=305',front:'./rail-boss-bruno-front-wreck192.webp?v=305&b=305',middle:'./rail-boss-bruno-middle-wreck192.webp?v=305&b=305',rear:'./rail-boss-bruno-rear-wreck192.webp?v=305&b=305'},
 lincomparable:{engine:'./rail-boss-lincomparable-engine-wreck192.webp?v=305&b=305',front:'./rail-boss-lincomparable-front-wreck192.webp?v=305&b=305',middle:'./rail-boss-lincomparable-middle-wreck192.webp?v=305&b=305',rear:'./rail-boss-lincomparable-rear-wreck192.webp?v=305&b=305'}
};
...
const zubianGroup=createLazyImageGroup({atlas:'./zubian-atlas.webp?v=305'}),zubianArt=zubianGroup.images;
...
const cityGroup=createLazyImageGroup({london:'./terrain-city-london96.webp?v=305&b=305',berlin:'./terrain-city-berlin96.webp?v=305&b=305'}),cityArt=cityGroup.images;
   ...
   if(h.kind==='beam'){const x2=h.x+Math.cos(h.angle)*h.length,y2=h.y+Math.sin(h.angle)*h.length;
    if(h.visual==='apron-wire'){
     ...
     if(warning){// Fan telegraph only — same style as other bombing warnings, no sprite preview.
      ...
      c.fillStyle='#d2aa5a12';c.beginPath();c.moveTo(nozzle,0);c.arc(nozzle,0,h.length-nozzle,-half,half);c.closePath();c.fill();
      c.strokeStyle='#e6bc77';c.lineWidth=1.5;c.setLineDash([7,6]);c.beginPath();c.moveTo(nozzle,0);c.arc(nozzle,0,h.length-nozzle,-half,half);c.closePath();c.stroke();c.setLineDash([]);
      const prog=clamp((h.age-h.delay)/h.warning,0,1);
      c.strokeStyle='#f2d69b';c.beginPath();c.moveTo(nozzle,0);c.arc(nozzle,0,(h.length-nozzle)*prog,-half,half);c.closePath();c.stroke();}
     else{const activeAge=Math.max(0,h.age-h.delay-h.warning);
      // Cluster of small flame tongues that leave the muzzle one by one and
      // merge into a jet — not one continuous image stretched along the beam.
      const jetLen=(h.length-nozzle)*Math.min(1,activeAge/.45),blobs=Math.max(2,Math.round(jetLen/52));
      for(let i=0;i<blobs;i++){
       const d0=nozzle+jetLen*i/blobs,fli=Math.sin(h.age*23+i*2.1)*.5+.5,jy=Math.sin(h.age*31+i*3.7)*h.thickness*.16;
       const bh=h.thickness*(1.2+fli*.5)*(1-(i/blobs)*.3),fl=Math.min(96,jetLen/blobs*2.1);
       c.globalAlpha=.68+fli*.32;
       c.drawImage(img,d0,-bh/2+jy,fl,bh);
      }
      c.globalAlpha=1;}
