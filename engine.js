import {EnemyCollisionGrid} from './collision-grid.js?v=thermal-20260926-r1';
import {preparePersonalRound1918,advancePersonal1918,advanceBurns1918} from './pilot-lifecycle196.js?v=340';
import {installRevision} from './rebalance103.js?v=340&b=340&thermal=20260926r1';
import {installCloudCover} from './cloud-cover1.js?v=340&b=340&thermal=20260926r1';
import {installFleet} from './fleet-naval1.js?v=340&b=340&thermal=20260926r1';
import {installTrenchWar} from './trench-war1.js?v=340&b=340&thermal=20260926r1';
import {installCityAir} from './city-air1.js?v=340&b=340&thermal=20260926r1';
...
let max=this.sunStrikeContains(e)||e.stationary?0:e.bossDash>0?0:e.heavyBomber?.3:e.type==='boss'?2.0:e.type==='hunter'?1.7:e.type==='zeppelin'?.42:.85,steering=e.type==='bomber'?{turn:max>0?(e.heavyBomber?.12:.16):0,delta:0}:this.dogfightSteering(e,contact,dt,max);if(e.type==='bomber')steering.delta=Math.cos(this.t*.6+e.wobble)*steering.turn*dt;e.a+=Math.max(-steering.turn*dt,Math.min(steering.turn*dt,steering.delta));const sunlightSpeed=this.sunStrikeContains(e)?SUN_STRIKE.speedMultiplier:1;e.x+=Math.cos(e.a)*e.speed*energySpeed*(e.bossDash>0?ENEMY_MOVEMENT_BALANCE.dashMultiplier:1)*sunlightSpeed*dt;e.y+=Math.sin(e.a)*e.speed*energySpeed*(e.bossDash>0?ENEMY_MOVEMENT_BALANCE.dashMultiplier:1)*sunlightSpeed*dt;if(!e.surface&&!e.stationary){const bodies=this.stageBoss?.stages?.encounter?.bodies;if(bodies)for(const body of bodies.values()){if(body.dead)continue;for(const part of body.parts.values()){if(part.destroyed)continue;const dx=e.x-(body.x+part.x),dy=e.y-(body.y+part.y),d=Math.hypot(dx,dy),r=(part.radius||30)+30;if(d<r&&d>0.01){e.x+=dx/d*(r-d)*.6;e.y+=dy/d*(r-d)*.6}}}}if(e.type==='zeppelin'&&!e.summonDone&&e.hp<=e.maxHp*.52){e.summonDone=true;this.event('wave','비행선 호위 함대 전개');this._escortSummon=true;for(let i=0;i<4;i++){const before=this.enemies.length;this.spawnEnemy('zeppelin');const z=this.enemies.length>before?this.enemies.at(-1):null;if(z){const za=e.a+i*Math.PI/2+Math.PI/4;z.x=e.x+Math.cos(za)*170;z.y=e.y+Math.sin(za)*170;z.faction=e.faction;z.summonDone=true;z.fire=2.2+i*.6}}this._escortSummon=false}if(!this.sunStrikeContains(e)){e.fire-=dt*(e.vossSurge?1.25:1);if(e.fire<0)this.fireEnemy(e);}if(!e.surface)for(const p of this.patrols||[])if(p.hp>0&&Math.hypot(e.x-p.x,e.y-p.y)<24)this.hitPatrol(p,18);if(!e.surface&&Math.hypot(e.x-this.x,e.y-this.y)<(e.contactRadius??(e.heavyBomber?48:e.type==='boss'?ENEMY_MOVEMENT_BALANCE.aceContactRadius:e.type==='zeppelin'?42:19)))this.hit(e.type==='boss'?22:e.type==='zeppelin'?18:15)}
 ...
 this.drops=this.drops.filter(d=>!d.dead&&(d.life===undefined||d.life>0)&&Math.hypot(d.x-this.x,d.y-this.y)<1500).slice(-250);for(let p of this.particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.life-=dt}this.particles=this.particles.filter(p=>p.life>0).slice(-(this.visualParticleLimit||450));
...
// evade/skill/boost bursts freeze the measured value rather than inflating it.
...
// Build once after movement. Query order remains the original enemies array order.
Game.prototype.prepareProjectileCandidates=function(){
 if(this.useCollisionGrid===false){this._projectileGrid?.clear();return;}
 (this._projectileGrid??=new EnemyCollisionGrid(enemyAircraftHitRadius)).build(this.enemies);
};
Game.prototype.projectileCandidates=function(b){
 return this.useCollisionGrid===false||!this._projectileGrid?this.enemies:this._projectileGrid.query(b,this.enemies);
};
...
Game.prototype.specialRoundImpact=function(b,e){
 ...
 // Small hull/water contact spray is visual only; never queues area damage.
 if((e.navalVessel||e.type==='ship')&&!b.actualExplosion&&!b.fireZone&&!b.rocket&&Math.hypot(b.vx||0,b.vy||0)>0){
  world.combatFX??=[];if(world.combatFX.length<160){const medium=!!(b.cow37||b.motorCannon||b.heavy);world.combatFX.push({x:b.x??e.x,y:b.y??e.y,radius:medium?26:11,life:medium?.42:.24,maxLife:medium?.42:.24,fxSource:medium?'navalMedium':'navalMG',fxOnly:true})}
 }
 if(b.upgradeRocket&&!b.rocketDetonated){b.rocketDetonated=true;this.queueExplosionDamage(b.x,b.y,84,b.damage*.5,{exclude:e,fxSource:'lePrieur'})}
